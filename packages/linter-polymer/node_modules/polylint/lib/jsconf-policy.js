...
(function (Type) {
    ...
    Type[Type["BANNED_DEPENDENCY"] = 2] = "BANNED_DEPENDENCY";
    Type[Type["BANNED_NAME"] = 3] = "BANNED_NAME";
    Type[Type["BANNED_PROPERTY"] = 4] = "BANNED_PROPERTY";
    Type[Type["BANNED_PROPERTY_READ"] = 5] = "BANNED_PROPERTY_READ";
    Type[Type["BANNED_PROPERTY_WRITE"] = 6] = "BANNED_PROPERTY_WRITE";
    Type[Type["RESTRICTED_NAME_CALL"] = 7] = "RESTRICTED_NAME_CALL";
    Type[Type["RESTRICTED_METHOD_CALL"] = 8] = "RESTRICTED_METHOD_CALL";
    Type[Type["BANNED_CODE_PATTERN"] = 9] = "BANNED_CODE_PATTERN";
    Type[Type["BANNED_PROPERTY_CALL"] = 10] = "BANNED_PROPERTY_CALL";
        ...
        for (var i = 0, n = x.length; i < n; ++i) {
            ...
            if (!(el instanceof RegExp)) {
                ...
                try {
                    ...
                    console.error(e);
                    throw new Error('Failed to convert Java-style regex to JavaScript: ' +
                        javaSource + ' : ' + e);
                }
            }
            arr[i] = el;
...
class Requirement {
    constructor(spec) {
        ...
        this.exclude = optionalPathFilterFunction({ values: spec.whitelist, regexps: spec.whitelist_regexp });
        /** @type {Function<string>:boolean|null} */
        this.include = optionalPathFilterFunction({ values: spec.only_apply_to, regexps: spec.only_apply_to_regexp });
        if (this.include && this.exclude) {
            throw new Error('Requirement cannot specify both whtielist* and only_apply_to*: ' +
                JSON.stringify(spec));
        }
        /** @type {number|null} */
        ...
        if ((this.type === Type.CUSTOM) !== (this.js_module !== null)) {
            throw new Error('Only/all custom requirements may/must have a js_module: ' +
                JSON.stringify(spec));
        }
        /** @type {!Array.<string>} */
...
class Policy {
     ...
     */
    applicableTo(path) {
        return this.requirements.filter(function (requirement) {
            return ((!requirement.include || requirement.include(path)) &&
...
function fromRequirements(conformanceConfig) {
    ...
    if (!Array.isArray(requirementSpecs)) {
        throw new Error('missing requirement array: ' + JSON.stringify(conformanceConfig));
    }
...
exports.fromRequirements = fromRequirements;
