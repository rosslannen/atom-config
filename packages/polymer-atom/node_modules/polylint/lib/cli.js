...
// jshint esversion: 6
'use strict';
const polylint_1 = require("./polylint");
const jsconf_policy = require("./jsconf-policy");
    ...
    return new Promise(function (resolve, reject) {
        ...
        if (!inputs || !inputs.length) {
            if (options['config-file'] && options['config-field']) {
                ...
                try {
                    ...
                    let contents = fs.readFileSync(options['config-file']);
                    ...
                    logger.error("No input specified and no '" + field + "' found in '" +
                        options['config-file'] + "'!");
                    inputsOk = false;
                }
            }
        ...
        if (options.policy) {
            jsconfPolicyPromise = new Promise(function (fulfill, reject) {
                fs.readFile(options.policy, { encoding: 'utf8' }, function (err, fileContent) {
                    if (err) {
                        reject(err);
                    }
                    else {
        ...
        inputs.forEach(function (input) {
            ...
            lintPromise = lintPromise.then(function () {
                return polylint_1.polylint(input, {
                    root: root,
