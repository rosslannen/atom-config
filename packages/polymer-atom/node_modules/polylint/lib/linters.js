...
const lint_error_1 = require("./lint-error");
const expressions_1 = require("./expressions");
var expressionParser = new expressions_1.ExpressionParser();
...
function domModuleTemplates(analyzer, id) {
    ...
    var templates = [];
    var isOuterTemplate = p.AND(isTemplate, p.NOT(parentIsTemplate));
...
function textNodesInDomModuleTemplates(analyzer, id) {
    ...
    templates.forEach(function (template) {
        textNodes = textNodes.concat(dom5.nodeWalkAll(template, p.AND(dom5.isTextNode, p.NOT(twoTemplateAncestors))));
    });
    return textNodes;
}
/**
...
function lintErrorFromNode(node, message) {
    return new lint_error_1.LintError(node.__ownerDocument, node.__locationDetail, message, true);
}
function lintErrorFromJavascript(htmlNode, javascriptNode, href, message) {
    ...
    return new lint_error_1.LintError(href, newLoc, message, true);
}
function lintBindingExpression(parsed, properties, href, node, javascriptNode, expressionName, is, implicitBindings, name) {
    ...
    boundVariablesDeclared: function findBindingToClass(analyzer) {
        ...
        analyzer.elements.forEach(function (element) {
            ...
            expressions.forEach(function (expression) {
                undeclaredBindings =
                    undeclaredBindings.concat(lintBindingExpression(expression.parsed, element.properties, element.contentHref, expression.node, undefined, expression.name, element.is, implicitBindings, "Computed Binding"));
            });
            if (element.observers) {
                ...
                parsedObservers.forEach(function (observer) {
                    undeclaredBindings =
                        undeclaredBindings.concat(lintBindingExpression(observer.parsed, element.properties, element.contentHref, observer.node, observer.javascriptNode, observer.name, element.is, implicitBindings, "Observer"));
                });
            }
        });
        return undeclaredBindings;
                    ...
                    element.properties.forEach(function (property) {
                        ...
                        if (property.name == observer) {
                            ...
                            if (property.type === "Function")
                                return;
                            badObservers.push(lintErrorFromJavascript(element.scriptElement, property.javascriptNode, element.contentHref, "Observer '" + observer + "' is not a function."));
                        }
                    });
                    if (!foundObserver) {
                        badObservers.push(lintErrorFromJavascript(element.scriptElement, observerNode, element.contentHref, "Observer '" + observer + "' is undefined."));
                    }
                }
            });
        }.bind(this));
